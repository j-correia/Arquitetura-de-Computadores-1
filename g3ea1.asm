...
main:	ori $t0, $0, 0			# unsigned int res = 0
	...
	andi $t2, $v0, 0x0F		# mdor = $v0 & 0x0F
	ori $v0, $0, read_int		# mdo = read_int()
	syscall				# mdo = read_int()
	andi $t3, $v0, 0x0F		# mdo = $v0 & 0x0F
while:	beq $t2, 0, endw		# if( mdor == 0)-> endw
	bge $t1, 4, endw		# if( i >= 4)-> endw
	addi $t1, $t1, 1		# i++	## incrementação feita no fim da análise
...
eif:	sll $t3, $t3, 1			# mdo = mdo << 1 
	...
	j while				# }
endw:	addi $t1, $t1, 1		# i++  ## por uma questão de coesao de tradução
